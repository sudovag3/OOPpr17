...
// Перегрузка оператора * для выполнения пересечения множеств
type_set operator*(const type_set& A, const type_set& B) {
    type_set I;
    // Используем функцию set_intersection для выполнения пересечения множеств A и B
...
int main() {
    ...
    // Выполняем пересечение множеств A и B с помощью оператора *
    type_set I = A * B;
